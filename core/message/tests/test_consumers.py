...
async def test_send_message_creates_message(room, communicator):
    ...
    assert await Message.objects.filter(room=room, text="Hello").aexists()
...
async def test_send_message_edit_message(communicator, message):
    ...
    await communicator.send_json_to(
        {"type": "message_edit", "message": "edited", "message_id": message.id}
    )
    ...
    assert await Message.objects.filter(id=message.id, text="edited").aexists()
...
async def test_send_message_read(communicator, message):
    ...
    assert await MessageRead.objects.filter(message=message).aexists()
