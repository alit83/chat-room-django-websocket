...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        try:
            ...
            self.room = await Room.objects.aget(
                pk=self.room_id, participants=self.user.pk
            )
            ...
            await self.accept()
            # A user may have multiple browser tabs open. We track the
            # number of active connections so the user is considered offline
            # only after the last WebSocket disconnects.
    ...
    async def handle_message(self, data):
        ...
        if not text or not text.strip():
            await self.send(
                text_data=json.dumps({"error": "Message is required"})
            )
            return
        ...
        participants = await MessageService.get_other_participants(
            room=self.room, user_pk=self.user.pk
        )
    ...
    async def handle_typing(self, data):
        ...
        if not isinstance(is_typing, bool):
            await self.send(
                text_data=json.dumps({"error": "is_typing must be boolean."})
            )
            return
        await self.channel_layer.group_send(
            self.room_group_name,
    ...
    async def handle_read(self, data):
        ...
        if not serializer.is_valid():
            await self.send(
                text_data=json.dumps({"errors": serializer.errors})
            )
            return
        message_ids = serializer.validated_data["message_ids"]
        try:
            read_messages = await MessageService.read_messages(
                message_ids=message_ids, room=self.room, user_pk=self.user.pk
            )
    ...
    async def broadcast_presence(self, connections):
        ...
        if connections == 1:
            ...
            # broadcasts the presence for every room that user is a
            # participants in
    ...
    async def handle_message_edit(self, data):
        ...
        if not message:
            await self.send(
                text_data=json.dumps({"error": "message not found"})
            )
            return
        await self.channel_layer.group_send(
            self.room_group_name,
    ...
    async def handle_message_delete(self, data):
        ...
        message_ids = serializer.validated_data["message_ids"]
        try:
            await MessageService.delete_messages(
                message_ids=message_ids, room=self.room, user_pk=self.user.pk
            )
        except ValidationError as exc:
            await self.send(
