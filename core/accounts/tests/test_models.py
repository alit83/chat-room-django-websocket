...
def test_create_superuser_successfully():
    super_user = User.objects.create_superuser(username='testsuperuser',password='@a12345678')
    assert super_user.is_superuser
    assert super_user.username == 'testsuperuser'

def test_create_superuser_unsuccessfully():
    with pytest.raises(ValueError):
        User.objects.create_superuser(username='testsuperuser',password='@a12345678',is_superuser=False)
