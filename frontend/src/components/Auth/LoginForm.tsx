...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuth(access, refresh, { username, first_name: '', last_name: '', id: user_id })
      
      const profile = await authApi.getProfile()
      useAuthStore.getState().setUser({ ...profile, username, id: user_id })
      navigate('/')
    } catch (e: unknown) {
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-[var(--bg-primary)] px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-[var(--text-muted)]">Sign in to continue</p>
        </div>
